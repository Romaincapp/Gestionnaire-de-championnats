...
13. **❌ Showing the bib (dossard) as a bare number in lane mode** - in swimming, bibs are auto-assigned in seed-time order (1..5) while lanes follow "fastest in the middle" (3, 4, 2, 5, 1), so a bare bib reads as a lane number (user report 2026-09: "button 1 shows 5"). In lane mode the lane is the visible identifier everywhere: lane buttons (`laneButtonInnerHTML`, "Couloir" caption, no bib), race table first column (`raceIdCellContent`), live display, printed start list (sorted by lane). Keep the race table's cell count unchanged: inline editing targets cells by index (`cells[3..8]`). **The printed sheet and the lane buttons must read the same lanes**: missing lanes are completed by one shared function, `ensureSerieLanes(serie)` (`multisport.iife.js`), called both at race start and at print time — never complete lanes inline in only one of those paths. The print sheet must list series through `getEventSeries(chronoData, evt)` (nested `event.series` + flat `chronoData.series` linked by `eventId`), not `event.series` alone.
